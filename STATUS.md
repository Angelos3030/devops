...
### Το «ξενοδοχείο» έβγαινε γενικό site (11 Αυγούστου 2026)

Prompt «ξενοδοχείο» → site με επωνυμία «Ξενοδοχείο», παύλες αντί στοιχείων,
τέσσερις αριθμημένες υπηρεσίες χωρίς περιγραφή (μία, «Πρόσφυση», χωρίς νόημα)
και τροπικό resort ως hero. Το vertical matching ήταν **σωστό** (rooms → aegean)·
το περιεχόμενο ήταν τόσο άδειο που δεν ξεχώριζε από ξυλουργό. Πέντε αιτίες:

1. `db.create_client` έγραφε κυριολεκτικά `"—"` για ό,τι έλειπε. Οι στήλες είναι
   `NOT NULL DEFAULT ''` — η παύλα δεν χρειάστηκε ποτέ και έβγαινε στη σελίδα.
2. `quick_start.parse` ζητούσε υπηρεσίες από το AI ακόμη και για σκέτη κατηγορία.
   Ο κανόνας υπήρχε ήδη στο `site_copy.py`, αλλά το parse έτρεχε **πριν** και τον
   παρέκαμπτε: αποθήκευε υπηρεσίες, οπότε μετά ο guard έβλεπε `has_services=True`.
3. Οι υπηρεσίες αποθηκεύονταν με κενή περιγραφή και εκτόπιζαν τις ελεγμένες του
   vertical, που **έχουν** περιγραφή.
4. `_CITY_RE` με `IGNORECASE` κόλλαγε την επόμενη λέξη: «στην Πάρο με 12 δωμάτια»
   → πόλη «Πάρο με». Και η αιτιατική δεν διορθωνόταν ποτέ από το AI.
5. Χωρίς διεύθυνση/πόλη γεωκωδικοποιούσαμε το κενό → κέντρο Ελλάδας (38.99, 21.98)
   ως καρφίτσα και ως `geo` στο JSON-LD.

Επιπλέον, το `_DEFAULT_HERO["rooms"]` και το `hospitality` pool του
`mediaFallback.js` ήταν τροπικό resort — σωστό vertical, λάθος ήπειρος. Πέρασαν
σε κυκλαδίτικη κατεύθυνση, ίδια με το demo «Θαλασσιά». Το πέπλο του `aegean` hero
δυνάμωσε στο κέντρο: ήταν φτιαγμένο για σκούρες φωτογραφίες και το λευκό κείμενο
χανόταν πάνω στους ασβεστωμένους τοίχους.
