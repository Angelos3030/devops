...
## Κύκλος 13/8/2026 — Solo Practitioner → `signature`

| Πεδίο | Τιμή |
| --- | --- |
| Reference | `https://websitedemos.net/accountant-02/` (Astra starter, δημόσιο demo) |
| Κατηγορία | Accessible Source |
| Τι κρατήθηκε | **Σχεδιαστική λογική μόνο**: το πρόσωπο ως κύρια οπτική μονάδα· τυπογραφικός κατάλογος υπηρεσιών αντί για πλέγμα καρτών· ένα ήσυχο accent· η επαφή ως κλείσιμο αφήγησης, όχι ως φόρμα |
| Τι ΔΕΝ κρατήθηκε | Κανένας κώδικας, markup, shortcode, κείμενο, εικόνα, εικονίδιο ή γραμματοσειρά. Καμία παλέτα του reference — η ταυτότητα του `signature` παράχθηκε ανεξάρτητα και επαληθεύτηκε από τον spine guard |
| Γιατί δικαιολογείται νέο theme | Αλλάζει **αρχιτεκτονική πληροφορίας**, όχι ενότητα: η μονάδα εμπιστοσύνης είναι ο **άνθρωπος**, όχι η εταιρεία (`marble`), η δουλειά (`canvas`) ή το επείγον (`callout`). Κανένα υπάρχον theme δεν βάζει το όνομα ως `h1` και ως υπογραφή |
| Verticals | λογιστής, ψυχολόγος/θεραπευτής, διαιτολόγος, coach, φωτογράφος, ανεξάρτητος μεσίτης |

### Το πραγματικό εμπόδιο ήταν το no-photo, όχι το layout

Ένα theme προσωπικής μάρκας χωρίς φωτογραφία είναι το δυσκολότερο no-photo που
έχουμε λύσει: η κενή στήλη είναι ακριβώς εκεί που ο επισκέπτης ψάχνει πρόσωπο.

Δύο προτάσεις εξετάστηκαν:

1. **Μονόγραμμα + κάρτα ταυτότητας** — το γράμμα του ονόματος και οι *αληθινοί*
   παράμετροι που ο πελάτης έχει ούτως ή άλλως: ειδικότητα, περιοχές, ώρες, τηλέφωνο
2. **Μόνο τυπογραφία** — το όνομα σε πλήρες πλάτος, χωρίς δεύτερη στήλη

Επιλέχθηκε η **1**, με στοιχείο της 2: χωρίς πορτρέτο το όνομα μεγαλώνει
(`clamp(3.2rem, 8.5vw, 6.4rem)`) και παίρνει τον χώρο που θα έπαιρνε το πρόσωπο.

**Μετρημένο αποτέλεσμα:** no-photo = 104% του κειμένου της έκδοσης με φωτογραφία,
8 ενότητες έναντι 9, 3253px έναντι 3612px στα 1440.

### Ο κανόνας που προέκυψε: `HERO_IS_REAL`

Το `photos=none` **δεν** παρήγαγε ποτέ αληθινό no-photo — το `withMediaFallback`
ξαναβάζει stock hero και το `professional` set περιέχει φωτογραφία **ανθρώπων**.
Σε πλαίσιο πορτρέτου με `alt="{ΟΝΟΜΑ} — {ΕΙΔΙΚΟΤΗΤΑ}"` αυτό είναι κατασκευασμένη
ταυτότητα, το χειρότερο δυνατό ψέμα σε site προσώπου.

Το `MEDIA_MODE` δεν απαντούσε στο ερώτημα: με gallery αλλά χωρίς hero βγάζει
`mixed` ενώ το `HERO_IMAGE` είναι stock. Προστέθηκε `HERO_IS_REAL` στο
`mediaFallback` — προσθετικό, αδρανές για τα υπόλοιπα themes. Για τον ίδιο λόγο
το `signature` φιλτράρει τα `illustrative` gallery items: λωρίδα από stock
γραφεία σε προσωπική μάρκα διαβάζεται ως «η δουλειά μου», δηλαδή ως ψεύτικη απόδειξη.
