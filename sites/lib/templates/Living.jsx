...
export default function Living({ data: d }) {
  ...
  return (
    <div className={s.root}>
      <nav className={s.nav} aria-label="Κύρια πλοήγηση"><a href="#top" className={s.brand}><Brand data={d} /></a><div><a href="#materials">Υλικά</a><a href="#services">Υπηρεσίες</a></div><a href={tel} className={s.call}>Επικοινωνία</a></nav>
      <header id="top" className={s.hero}>
        <div className={s.heroCopy}><span className={s.eyebrow}>{d.KICKER}</span><h1>{d.NAME}</h1><p>{d.TAGLINE}</p><a href={tel} className={s.cta}>Μίλησε μαζί μας <span aria-hidden="true">↗</span></a></div>
      ...
      <main>
        <section id="materials" className={s.materials}><header><span className={s.eyebrow}>Ζωντανά υλικά</span><h2>Κάθε επιφάνεια έχει ιστορία.</h2></header><div className={s.materialGrid}>{gallery.slice(0,3).map((item,i)=><figure key={i}><img src={item.image} alt={item.title} loading="lazy" /><figcaption><span>0{i+1}</span><b>{item.title}</b><small>{item.sub}</small></figcaption></figure>)}{gallery.length===0&&services.slice(0,3).map((item,i)=><article key={i}><span>0{i+1}</span><h3>{item.title}</h3><p>{item.desc}</p></article>)}</div></section>

        <section id="services" className={s.services}><div className={s.serviceIntro}><span className={s.eyebrow}>Τι δημιουργούμε</span><h2>{d.STORY_TITLE}</h2><p>{d.INTRO || d.TAGLINE}</p></div><div className={s.petals}>{services.map((item,i)=><a href={tel} key={i} className={s.petal}><span>{item.num || String(i+1).padStart(2,'0')}</span><h3>{item.title}</h3><p>{item.desc}</p></a>)}</div></section>

        {gallery.length>3&&<section id="work" className={s.gallery}><header><span className={s.eyebrow}>Επιλεγμένα έργα</span><h2>Φτιαγμένα για να ζουν μαζί σου.</h2></header><div>{gallery.slice(3).map((item,i)=><figure key={i}><img src={item.image} alt={item.title} loading="lazy" /><figcaption>{item.title}</figcaption></figure>)}</div></section>}
